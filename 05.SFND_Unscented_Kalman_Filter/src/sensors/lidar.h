...
struct Ray {
    ...
    // 				  for example 0 radians is along xy plane and pi/2 radians is stright
    // up resoultion: the magnitude of the ray's step, used for ray casting, the smaller the more
    // accurate but the more expensive
...
struct Lidar {
    ...
    ~Lidar() {
        // pcl uses boost smart pointers for cloud pointer so we don't have to worry about manually
        // freeing the memory
    }

    void updateCars(std::vector<Car> setCars) { cars = setCars; }
