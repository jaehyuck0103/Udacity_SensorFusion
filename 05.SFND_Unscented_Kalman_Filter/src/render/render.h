...
struct accuation {
    ...
    accuation(long long t, float acc, float s) : time_us(t), acceleration(acc), steering(s) {}
};
...
struct Car {
    ...
    Car() : position(Vect3(0, 0, 0)), dimensions(Vect3(0, 0, 0)), color(Color(0, 0, 0)) {}

    Car(Vect3 setPosition,
        Vect3 setDimensions,
        Color setColor,
        float setVelocity,
        float setAngle,
    ...
    bool checkCollision(Vect3 point) {
        ...
        double xPrime =
            ((point.x - position.x) * cosNegTheta - (point.y - position.y) * sinNegTheta) +
            position.x;
        double yPrime =
            ((point.y - position.y) * cosNegTheta + (point.x - position.x) * sinNegTheta) +
            position.y;
