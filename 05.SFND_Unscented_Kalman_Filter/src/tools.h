...
struct lmarker {
    double x, y;
    lmarker(double setX, double setY) : x(setX), y(setY) {}
};
...
class Tools {
  public:
    ...
    rmarker radarSense(
        ...
        bool visualize);
    void
    ukfResults(Car car, pcl::visualization::PCLVisualizer::Ptr &viewer, double time, int steps);
    /**
     * A helper method to calculate RMSE.
     */
    VectorXd
    CalculateRMSE(const vector<VectorXd> &estimations, const vector<VectorXd> &ground_truth);
