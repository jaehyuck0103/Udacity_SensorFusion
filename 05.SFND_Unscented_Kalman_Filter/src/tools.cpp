...
rmarker Tools::radarSense(
    ...
    if (visualize) {
        viewer->addLine(
            ...
            pcl::PointXYZ(
                ego.position.x + marker.rho * cos(marker.phi),
...
void Tools::ukfResults(
    ...
    viewer->addArrow(
        ...
        pcl::PointXYZ(
            ukf.x_[0] + ukf.x_[2] * cos(ukf.x_[3]),
            ukf.x_[1] + ukf.x_[2] * sin(ukf.x_[3]),
            3.5),
        0,
        1,
        0,
...
VectorXd
Tools::CalculateRMSE(const vector<VectorXd> &estimations, const vector<VectorXd> &ground_truth) {
